...
class GetTokenPriceSuccess {
  final String tokenAddress;
  final Price price;
  GetTokenPriceSuccess({this.price, this.tokenAddress});
}
...
ThunkAction getTokenBalanceCall(Token token) {
  return (Store store) async {
    ...
    try {
      String walletAddress = store.state.userState.walletAddress;
      void Function(BigInt) onDone = (BigInt balance) async {
        ...
        await token?.fetchTokenLastestPrice(
            onDone: (price) {
              logger.info(
                  'getTokenBalanceCall for ${token.address} ${price.currency}');
              store.dispatch(UpdateCashToken(
                  tokenToUpdate: token?.copyWith(priceInfo: price)));
            },
            onError: (e) {
              logger.severe(
                  'ERROR while getTokenBalanceCall for ${token.address}');
            },
